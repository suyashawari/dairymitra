...
class FarmerService {
  // static const String _baseUrl = 'https://api-farmer1.onlinedairy.app';

  final String? _baseUrl = dotenv.env['API_KEY'];
  Future<List<PUser>> getAllFarmers() async {
    try {
      final response = await http.get(Uri.parse('$_baseUrl/v1/farmers'));
  ...
  Future<List<PUser>> getAllEmployees() async {
    try {
      final response = await http.get(Uri.parse('$_baseUrl/v1/employees'));
      if (response.statusCode == 200) {
        final List<dynamic> jsonList = jsonDecode(response.body);
        return jsonList.map((json) => PUser.fromJson(json)).toList();
      }
  ...
  Future<List<MilkRequest>> getAllMilkRequests() async {
    try {
      final response = await http.get(Uri.parse('$_baseUrl/v1/milk-requests'));
      if (response.statusCode == 200) {
        final List<dynamic> jsonList = jsonDecode(response.body);
  ...
  Future<bool> addMilkRequest(MilkRequest request) async {
    try {
      ...
      final response = await http.post(
        Uri.parse('$_baseUrl/v1/milk-requests'),
        headers: {'Content-Type': 'application/json'},
  ...
  Future<List<MilkRequest>> getMilkRequestsByFarmer(String email) async {
    try {
      final response = await http.post(
        Uri.parse('$_baseUrl/v1/milk-requests/by-farmer'),
        headers: {'Content-Type': 'application/json'},
