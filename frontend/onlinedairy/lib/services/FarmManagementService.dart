...
class FarmManagementService {
  // static const String _baseUrl = 'https://farm-mgmt.onlinedairy.net';

  final String? _baseUrl = dotenv.env['API_KEY'];
  Future<List<PUser>> getAllFarmers() async {
    try {
      final response = await http.get(Uri.parse('$_baseUrl/management/farmers'));
  ...
  Future<List<PUser>> getAllEmployees() async {
    try {
      final response = await http.get(Uri.parse('$_baseUrl/management/staff'));
      if (response.statusCode == 200) {
        final List<dynamic> jsonList = jsonDecode(response.body);
        return jsonList.map((json) => PUser.fromJson(json)).toList();
      }
  ...
  Future<bool> addFarmer(PUser farmer) async {
    try {
      final response = await http.post(
        Uri.parse('$_baseUrl/management/farmers/create'),
  ...
  Future<List<MilkRequest>> getAllMilkRequests() async {
    try {
      final response = await http.get(Uri.parse('$_baseUrl/management/milk'));
      if (response.statusCode == 200) {
        final List<dynamic> jsonList = jsonDecode(response.body);
  ...
  Future<bool> addMilkRequest(MilkRequest request) async {
    try {
      ...
      final response = await http.post(
        Uri.parse('$_baseUrl/management/milk/create'),
        headers: {'Content-Type': 'application/json'},
  ...
  Future<List<MilkRequest>> getMilkRequestsByFarmer(String email) async {
    try {
      final response = await http.post(
        Uri.parse('$_baseUrl/management/milk/filter'),
        headers: {'Content-Type': 'application/json'},
