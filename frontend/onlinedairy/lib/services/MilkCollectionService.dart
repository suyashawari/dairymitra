...
class MilkCollectionService {
  // static const String _baseUrl = 'https://milk-collect.onlinedairy.io';

  final String? _baseUrl = dotenv.env['API_KEY'];
  Future<List<PUser>> getAllFarmers() async {
    try {
      final response = await http.get(Uri.parse('$_baseUrl/collect/farmers'));
  ...
  Future<List<PUser>> getAllEmployees() async {
    try {
      final response = await http.get(Uri.parse('$_baseUrl/collect/employees'));
      if (response.statusCode == 200) {
        final List<dynamic> jsonList = jsonDecode(response.body);
        return jsonList.map((json) => PUser.fromJson(json)).toList();
      }
  ...
  Future<bool> addFarmer(PUser farmer) async {
    try {
      final response = await http.post(
        Uri.parse('$_baseUrl/collect/farmers/new'),
  ...
  Future<List<MilkRequest>> getAllMilkRequests() async {
    try {
      final response = await http.get(Uri.parse('$_baseUrl/collect/milk/all'));
      if (response.statusCode == 200) {
        final List<dynamic> jsonList = jsonDecode(response.body);
  ...
  Future<bool> addMilkRequest(MilkRequest request) async {
    try {
      ...
      final response = await http.post(
        Uri.parse('$_baseUrl/collect/milk/new'),
        headers: {'Content-Type': 'application/json'},
  ...
  Future<List<MilkRequest>> getMilkRequestsByFarmer(String email) async {
    try {
      final response = await http.post(
        Uri.parse('$_baseUrl/collect/milk/farmer'),
        headers: {'Content-Type': 'application/json'},
